...
*/
  ...
  */
    ...
    if (req.body.session_selected_a !== undefined) {
      for (
        let q = 0;
        q < req.body.session_selected_a.length;
        q++ //* loop through sessions selected and insert booking record for each session
      ) {
        Booking.findOne(
          {
            facility: "GF01",
    ...
    if (req.body.session_selected_b !== undefined) {
      for (
        let r = 0;
        r < req.body.session_selected_b.length;
        r++ //* loop through sessions selected and insert booking record for each session
      ) {
        Booking.findOne(
          {
            facility: "RB01",
    ...
    User.findOne({ email: res.locals.user.email }).then((user) => {
      ...
      user.save().then(() => {});
      //              console.log(">>> conf seq num after if stmt ==> ", user.conf_seq_num);
      //              console.log(">>> user after if stmt ==> ", user);
      req.flash(
        "success_msg",
        "Booking Added! Booking number is",
        booking_number
      );
      res.redirect("/bookings");
    });
  ...
  //* not "Addbooking" => find available slots and build table
    ...
    Booking.find(
      { facility: "GF01", bookingDate: req.body.bookingDate },
      { _id: 0, session: 1 }
    )
      .lean()
      .sort({ bookingDate: 1, session: 1 })
      .then((booked_sessions_a) => {
        ...
        /*            let booked_status_a = "";
            ...
            for (let i = 1; i < 10; i++) 
            { 
                if (booked_sessions_a.includes(i))
...
*/
        //*            console.log("@@@ sessions after for loop => ", sessions);
      });

    Booking.find(
      { facility: "RB01", bookingDate: req.body.bookingDate },
      { _id: 0, session: 1 }
    )
      .lean()
      .sort({ bookingDate: 1, session: 1 })
    ...
    Booking.findOne(
      ...
      function (err, result) {
        ...
        if (result !== null) {
          ...
          Booking.findOne({ _id: save_booking_id }).then((booking) => {
            console.log(booking);
